...
- 이슈 추적을 위해 로깅되어야할 정보들 고민
  ...
  - 구현은?
    ...
    - Filter, Interceptor, AOP 등을 활용하여 MDC에 필요한 정보 세팅
    - 어떤 방식으로 하던 하나의 기술로 일관성을 맞추는 것이 사이드 이펙트를 방지함. Filter면 Filter만 쓰고, Interceptor면 Interceptor만 쓰는 것. 
      Spring MVC 안에서 로깅 구조를 설계할 때 대부분 특정 메서드 실행 전/후로 로그 구성 작업이 많이 이루어지는데, 
