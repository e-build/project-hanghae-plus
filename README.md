...
- 트랜잭션의 범위를 최소화 했을 때의 장단점은?
  ...
  - 애플리케이션에서 작성되는 DB 관련 동작들을 논리적으로 하나의 연산으로 봐야한다면 트랜잭션으로 묶게 됨. DB 관련 동작이 아닌 부분으로 인해 트랜잭션이 길어지지는 않는 지 점검할 필요가 있음.
    ```java
    @Transactional
...
- Circuit Breaker 는 어떤 용도로 사용되는 지?
  ...
  - 최근 요청들을 감시하여 실패율을 평가하고, 지정된 조건에 따라 서킷을 오픈함. 서킷 오픈 시 요청을 차단하고 특정 시간 or 요청동안은 fallback 처리. 
